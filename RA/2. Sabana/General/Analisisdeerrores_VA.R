...
###########################################
pc.files <- mget(ls(pattern="*Error_VA")) #look for all files with "_pc" at end

columnselect<-function(df){
  #df[,c("A02","A02_1","A03","A03_1","A01","IdIntegrante")]
  df[,c("DEPARTAMENTO","MUNICIPIO","A01","IdIntegrante")]#Sisben IV
}
...
Compilado$TIPO=ifelse(grepl("VA_",Compilado$NOMBRE_ERROR),"Valor admisible","Flujo")

Compilado$NOMBRE_ERROR=gsub("^VA_","",Compilado$NOMBRE_ERROR)
...
write.table(Compilado[Compilado$TIPO=="Valor admisible",], paste0("Analisisdeerrores_VA_",Fecha,".txt"), sep="|", row.names = FALSE)
